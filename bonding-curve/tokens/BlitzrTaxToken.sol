...
// Queried back against launchManager (the BlitzrBondingCurve factory) so this contract enforces
...
// into 1-4 creator-chosen tokens (drawn from BlitzrBondingCurve's owner-managed whitelist);
...
contract BlitzrTaxToken is IBlitzrLaunchToken {
    ...
    function _executeTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        uint256 contractAmt; // liquidity + marketing, swapped to BNB
        if (isBuy) {
    ...
    function _autoSwap(uint256 tokenAmount) private lockSwap {
        ...
        _safeSendBNB(marketingWallet, gotBNB - bnbLP);
    }
    ...
    // proportionally by their BlitzrTaxToken balance. Funds always land with the token's
    ...
    // Single-hop if the reflection token is WETH, otherwise this-token → WETH → token.
    function _swapForReflectionToken(address token, uint256 tokenAmount) private {
        ...
        _approve(address(this), address(pancakeRouter), tokenAmount);
        // amountOutMin = 0: same rationale as _swapTokensForBNB.
