...
interface ITaxInit {
    // Mirrors BlitzrTaxToken.TaxConfig field-for-field — Solidity ABI-encodes structs by
...
///         and buy/sell/migrate execution, and DEX migration (V2-and-burn for BlitzrTaxToken,
///         V3-and-lock-in-BlitzrLocker for BlitzrStandardToken).
///         There is no cross-contract trust boundary — a single owner governs everything.
///         Not upgradeable by design: a normal immutable contract.
contract BlitzrBondingCurve {
    ...
    struct TokenConfig {
        ...
        // V2 pair (BlitzrTaxToken) or V3 pool (BlitzrStandardToken) — both are created, and
        ...
        bool useV3; // BlitzrStandardToken migrates to a V3 1% pool, locked in the shared BlitzrLocker;
                    // BlitzrTaxToken keeps the existing V2-and-burn path.
    }
    ...
    struct CreateTTParams {
        ...
        uint256      antibotBlocks;
        bytes32      salt;
        // Tax config — fixed for the token's lifetime; see BlitzrTaxToken.sol.
        uint256      buyLiquidityTax;
        uint256      buyReflectionTax;
        uint256      buyMarketingTax;
        uint256      sellLiquidityTax;
    ...
    // ── BlitzrTaxToken reflection-token allowlist ────────────────────────────────
    ...
    function createToken(BaseParams memory p) external payable nonReentrant returns (address token) {
        ...
        uint256 tradingBlock_ = _registerToken(
            token, msg.sender, a, vBNB, migTarget, pool, true, p.enableAntibot, p.antibotBlocks
        );
    ...
    function createTT(CreateTTParams memory p) external payable nonReentrant returns (address payable token) {
        ...
        // BlitzrTaxToken keeps the existing V2-and-burn migration path (useV3 = false).
        uint256 tradingBlock_ = _registerToken(
            token, msg.sender, a, vBNB, migTarget, ITaxInit(token).pancakePair(), false, p.enableAntibot, p.antibotBlocks
        );
        emit TokenCreated(token, msg.sender, a.supply, vBNB, migTarget, p.enableAntibot, tradingBlock_);
    ...
    // BlitzrTaxToken itself at init, callable back against launchManager) — creators can't
    ...
    function setUsdQuotePair(address pair_) external onlyOwner {
        _validateUsdQuotePair(usdcToken, pair_);
        emit UsdQuotePairUpdated(usdQuotePair, pair_);
        usdQuotePair = pair_;
    }
    ...
    // BlitzrTaxToken path — LP tokens sent to the dead wallet, permanently locked.
    ...
    // _createV3Pool, run at createToken() time); this just wraps the raised BNB, mints a
    // full-range position directly to the shared contracts/BlitzrLocker.sol, and locks it
    // there permanently, same as every V3 Blitzr launch. The creator earns an ongoing share
    // of the pool's own 1 % trading fees instead of the LP being burned outright.
    ...
    // curve's cap guarantees migration raises exactly migrationTarget_ BNB by construction, so
    // this is the exact price the pool will actually receive liquidity at, not an estimate.
    function _createV3Pool(
        address token_, uint256 migrationTarget_, uint256 liqTokens
    ) private returns (address pool) {
        address weth_ = IPancakeRouter02(pancakeRouter).WETH();
        (address token0, address token1, uint256 amount0, uint256 amount1) = token_ < weth_
            ? (token_, weth_,        liqTokens, migrationTarget_)
            : (weth_,  token_, migrationTarget_, liqTokens);
